...
async def player_init(interaction: discord.Interaction, usr: discord.User):
    await interaction.response.send_message(f"*act like your account in initialized while it's really not in database*")
...
async def player_config(interaction: discord.Interaction, enemy: str):
    await interaction.response.send_message(f"*act like this opened the config panel*")
